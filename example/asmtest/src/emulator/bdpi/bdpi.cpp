...
void init() {
	...
	while(fmmap && !feof(fmmap)) {
		...
		if ( rbuf[0] == '+' ) { // load file
			...
			if ( !fdin ) {
				printf( "No memory file (%s) supplied\n", tok );
				continue;
			}
...
extern "C" void memWrite(uint32_t addr, uint32_t bytes, uint32_t data, uint32_t cycles) {
	init();
	if ( g_answermap.find(addr) != g_answermap.end() ) {
		int correct = g_answermap[addr];
		int given = *(int*)&data;
		printf( "\t\t** SYSTEM: Memory write request at cycle %d\n", cycles );
		if ( correct == given ) {
			printf( "\t\t** Results CORRECT! Writing %d to address %x\n", given, addr );
		} else {
